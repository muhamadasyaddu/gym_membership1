...
<div class="page-header">
    <div class="d-flex align-items-center mb-2">
        ...
        <h1 class="page-title mb-0">Edit Transaksi</h1>
    </div>
    <p class="page-subtitle">Perbarui data transaksi</p>
</div>
        ...
        <form method="POST" action="{{ route('transaksi.update', $transaksi) }}">
            ...
            <div class="row">
                <div class="col-md-4 mb-3">
                    ...
                    <input type="date" class="form-control @error('waktu_mulai') is-invalid @enderror" 
                           id="waktu_mulai" name="waktu_mulai" value="{{ old('waktu_mulai', $transaksi->waktu_mulai->format('Y-m-d')) }}">
                    @error('waktu_mulai')
                    <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
            ...
            <div class="row">
                <div class="col-md-4 mb-3">
                    ...
                    <select class="form-select @error('payment_method') is-invalid @enderror" 
                        ...
                        <option value="tunai" {{ old('payment_method', $transaksi->payment_method) == 'tunai' ? 'selected' : '' }}>Tunai</option>
                        <option value="e-wallet" {{ old('payment_method', $transaksi->payment_method) == 'e-wallet' ? 'selected' : '' }}>E-Wallet</option>
                    </select>
            ...
            <div class="mb-3">
                ...
                <textarea class="form-control @error('keterangan') is-invalid @enderror" 
                          id="keterangan" name="keterangan" rows="2">{{ old('keterangan', $transaksi->keterangan) }}</textarea>
                @error('keterangan')
                <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
